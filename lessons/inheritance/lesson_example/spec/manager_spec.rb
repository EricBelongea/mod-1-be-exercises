...
RSpec.describe Manager do
  describe "setup" do
    it "exists" do
      manager = Manager.new("Charles", 9876, 12)
  ...
  describe "Ability to have name and ID, like any other Employee" do
    # Write tests to prove that a Manager instance can have a Name and ID number, 
    # like any other Employee.
    it 'has name and id' do
      manager = Manager.new("Charles", 9876, 12)
      expect(manager.name).to eq("Charles")
      expect(manager.id).to eq(9876)
    end
    # Also, a Manager should be able to receive a total compensation. Write at least one test that 
    # proves they can access this method.
    it 'can recieve compensation' do
      manager = Manager.new("Charles", 9876, 12)
